...
function useTargetRect(targetId, stepIndex) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const maxH = Math.max(140, vh - TOP_RESERVE - BOTTOM_RESERVE);
    ...
    if (el && typeof el.scrollIntoView === 'function') {
      const vh = window.innerHeight;
      const maxH = Math.max(140, vh - TOP_RESERVE - BOTTOM_RESERVE);
      const tall = el.getBoundingClientRect().height > maxH;
    ...
    aliveInterval = setInterval(() => {
      schedule();
      if (Date.now() - start > 900) clearInterval(aliveInterval);
    }, 45);
...
export default function OnboardingTour({ user, onClose }) {
  ...
  const placement = computeTooltipPlacement(rect);
